...
\addbibresource{03_referencias_plan_tesis.bib}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \node[anchor=north west,inner sep=0pt]
      ...
      {\includegraphics[width=0.12\paperwidth]{03_logo_UNI.png}};
  \end{tikzpicture}
  \centering
...
\begin{tikzpicture}
  ...
  \node[rounded corners=4pt,draw=darkblue,fill=warmgray,
    ...
    {{\centering\textbf{Base física}\par}\vspace{0.5pt}\hyphenpenalty=10000\exhyphenpenalty=10000 La ampacidad $\Imax$ y $\Tmax$ dependen del cable y de su entorno térmico: suelo nativo, lecho de asiento y suelo de relleno.};
\end{tikzpicture}
...
\node[leaf,fill=lightblue] (materiales) at (5.6,4.68) {Materiales\\lecho de asiento\\suelo de relleno · zona seca};
...
\node[rounded corners=4pt,draw=unired,fill=softred,text width=0.94\linewidth,
 ...
 {\textbf{Síntoma:} más cables enterrados quedan expuestos a suelos nativos, suelos de relleno e interfaces diversas que condicionan su capacidad térmica y su ampacidad (corriente máxima).};
\end{tikzpicture}
\column{0.37\textwidth}
\begin{tikzpicture}[node distance=0.10cm,
...
\node[card,fill=lightblue,draw=midblue] (a) {\textbf{Evidencia publicada}\par Cuatro estudios independientes reportan cambios de ampacidad al variar las propiedades térmicas del suelo nativo o del suelo de relleno.};
\node[card,fill=softorange,draw=orange,below=of a] (b) {\textbf{Tipo de situación}\par Sistema físico-térmico heterogéneo: la humedad, los estratos, los suelos de relleno y las zonas secas cambian $k$ y la evacuación del calor.};
...
\node[card,fill=softgreen,draw=teal] (heterogeneity) at (4.78,2.02)
  {\textbf{3. Entorno heterogéneo}\par
   Cuatro estudios (MT--220 kV): al disminuir $k$ en los rangos estudiados, $\Imax$ cae entre \textbf{5\pct{} y 57\pct}. Influyen el suelo nativo, el suelo de relleno, la humedad y las interfaces.};
...
\node[rounded corners=4pt,draw=orange,fill=softorange,text width=6.65cm,
  ...
  {\textbf{Heterogeneidad:} humedad · estratos · lecho de asiento · suelo de relleno · interfaces · zonas secas};
...
\vspace{0.004\paperheight}
\makebox[\linewidth][c]{\includegraphics[width=1.08\linewidth,height=0.43\paperheight,keepaspectratio]{03_enescu_trench_xsection_es.png}}
\end{columns}
...
\node[tax,fill=softgreen,draw=teal] at (4.65,0)
  {\textbf{Entorno:} lecho de asiento, suelo de relleno, suelo nativo, interfaces y humedad.};
\end{tikzpicture}
...
\begin{scope}[shift={(4.65,0.42)}]
  ...
  \node[font=\fontsize{6.8}{7.7}\selectfont,align=center,text width=3.85cm] at (0,-0.65)
    {Suelo de relleno conductor o zona seca de baja $k$ cerca del cable.};
\end{scope}
...
\end{tikzpicture}
\source[\fontsize{6.0}{6.9}\selectfont]{Modelo operativo y diseño discreto de casos del plan; geometrías de referencia basadas en \textcite{enescu2021} y \textcite{kim2025}; estructura de archivos del proyecto.}
...
% 13 — Gráfico TikZ del informe rev1, figura arquitectura-pinn-lawal.
\begin{frame}{Estrucura de las redes PINN}
\begingroup
\definecolor{lightgray}{HTML}{F2F2F2}
\begin{tikzpicture}[remember picture,overlay]
  \node[anchor=north west,inner sep=0pt]
    at ([xshift=0.55cm,yshift=-0.96cm]current page.north west) {%
    \resizebox{7.8cm}{!}{%
\begin{tikzpicture}[
  font=\small,
...
\end{tikzpicture}
  ...
  \node[anchor=north west,inner sep=0pt,text width=6.25cm,align=left,
    font=\fontsize{8.5}{10.2}\selectfont]
    at ([xshift=8.85cm,yshift=-1.03cm]current page.north west)
    {\textcolor{darkblue}{\textbf{1. Aproximar la solución}}\\[2pt]
     La red estima $u,v$ a partir de $(x,y,t)$.};
  \node[anchor=north west,inner sep=0pt,text width=6.25cm,align=left,
    font=\fontsize{8.5}{10.2}\selectfont]
    at ([xshift=8.85cm,yshift=-2.05cm]current page.north west)
    {\textcolor{darkblue}{\textbf{2. Calcular derivadas}}\\[2pt]
     Deriva la misma red automáticamente.};
  \node[anchor=north west,inner sep=0pt,text width=6.25cm,align=left,
    font=\fontsize{8.5}{10.2}\selectfont]
    at ([xshift=8.85cm,yshift=-3.07cm]current page.north west)
    {\textcolor{darkblue}{\textbf{3. Evaluar la física}}\\[2pt]
     Mide el incumplimiento de la PDE: $f$.};
  \node[anchor=north west,inner sep=0pt,text width=6.25cm,align=left,
    font=\fontsize{8.5}{10.2}\selectfont]
    at ([xshift=8.85cm,yshift=-4.09cm]current page.north west)
    {\textcolor{darkblue}{\textbf{4. Ajustar los parámetros}}\\[2pt]
     Minimiza la pérdida total: física y datos.};
  \node[anchor=north west,rounded corners=4pt,draw=teal,fill=softgreen,
    inner sep=5pt,text width=5.90cm,align=left,
    font=\fontsize{8.4}{10.1}\selectfont]
    at ([xshift=8.85cm,yshift=-5.16cm]current page.north west)
    {\textcolor{teal}{\textbf{En esta tesis: conducción de calor}}\\[2pt]
     $(x,y)\longmapsto T(x,y)$; régimen estacionario.\\[2pt]
     $r_f=\nabla\!\cdot\!\left(k\nabla T\right)+Q$. La pérdida incluye ecuación de calor, contornos e interfaces.};
  \node[anchor=north west,inner sep=0pt,text width=7.75cm,align=left,
    font=\fontsize{6.8}{8}\selectfont,text=textgray]
    at ([xshift=0.65cm,yshift=-6.78cm]current page.north west)
    {$I$: operador identidad; $t$: tiempo en el esquema general.};
  \node[anchor=north west,rounded corners=4pt,draw=midblue!50,fill=lightblue!45,
    inner sep=3pt,text width=14.23cm,align=left,
    font=\fontsize{7.0}{8.2}\selectfont]
    at ([xshift=0.65cm,yshift=-7.14cm]current page.north west)
    {\textbf{Multiescala (borrador):} redes por material y mixtas; representaciones locales, Fourier y multipolares.\\[1pt]
     \textbf{Colocación:} uniforme, mezcla geométrica, bandas de interfaz y adaptativa por residuo.\\[1pt]
     \textbf{Desempeño:} escalas, pesos de pérdida, continuidad de $T$/flujo y verificación frente a FEM.};
\end{tikzpicture}
\endgroup
\source[\fontsize{5.8}{6.7}\selectfont]{Informe rev1, fig.~4.4: \textcite[fig.~1]{lawal2022}; \textcite[686--690]{raissi2019}; borrador de tesis, cap.~3.}
\note{Leer el gráfico de izquierda a derecha. La red aproxima las variables físicas y la diferenciación automática calcula las derivadas de esa misma aproximación; no se entrena una segunda red para derivar. El operador I es la identidad y no la corriente eléctrica. El residuo mide cuánto se incumple la ecuación gobernante. La pérdida de datos compara con referencias disponibles; la pérdida física penaliza la ecuación y condiciones auxiliares. Su combinación ponderada guía el ajuste de pesos y sesgos. El diagrama es general: en la tesis se usan coordenadas espaciales y la salida T(x,y), sin evolución temporal. La ecuación se aplica por material y las interfaces imponen continuidad de temperatura y flujo. La franja inferior sintetiza el capítulo 3 del borrador, secciones de selección de arquitectura, distribución de colocaciones y entrenamiento: el tamaño, las escalas locales, la representación, los pesos y el muestreo interactúan. Más colocaciones o un residuo pequeño no garantizan exactitud. Se requieren evaluación por material, continuidad de temperatura y flujo, balances y comparación independiente con FEM. Las alternativas bibliográficas VPINN y FBPINN no se presentan como resultados ejecutados de esa campaña. Tiempo sugerido: 75 segundos.}
\end{frame}


% 14 — Síntesis del marco térmico del informe rev1.
\begin{frame}{Ampacidad de cables eléctricos}
\begin{tikzpicture}[remember picture,overlay]
  \node[anchor=north west,inner sep=0pt,text width=14.5cm,align=left,
    font=\fontsize{11}{13}\selectfont,text=darkblue]
    at ([xshift=0.70cm,yshift=-1.08cm]current page.north west)
    {\textbf{Modelo 2D estacionario:} el calor generado se disipa por el cable y el suelo.};
  \node[anchor=north west,draw=midblue,fill=lightblue!45,rounded corners=5pt,
    inner sep=8pt,text width=6.45cm,minimum height=3.60cm,align=left,
    font=\fontsize{10}{12}\selectfont]
    at ([xshift=0.70cm,yshift=-2.00cm]current page.north west)
    {\textcolor{darkblue}{\textbf{1. Calcular la temperatura}}\\[9pt]
     $\displaystyle
      \frac{\partial}{\partial x}\!\left(k\frac{\partial T}{\partial x}\right)
      +\frac{\partial}{\partial y}\!\left(k\frac{\partial T}{\partial y}\right)+Q=0$
     \\[10pt]
     $T(x,y)$: temperatura.\\[2pt]
     $k(x,y)$: conductividad térmica.\\[2pt]
     $Q(x,y;I)$: calor por unidad de volumen.};
  \node[anchor=north west,draw=teal,fill=softgreen,rounded corners=5pt,
    inner sep=8pt,text width=6.45cm,minimum height=3.60cm,align=left,
    font=\fontsize{10}{12}\selectfont]
    at ([xshift=8.12cm,yshift=-2.00cm]current page.north west)
    {\textcolor{teal}{\textbf{2. Encontrar la ampacidad}}\\[9pt]
     $\displaystyle\Imax=\max\{I:\Tmax(I)\leq T_{\mathrm{lim}}\}$
     \\[10pt]
     $\Tmax$: temperatura máxima del conductor.\\[2pt]
     $T_{\mathrm{lim}}$: límite térmico del cable.\\[2pt]
     Ajustar $I$ hasta alcanzar ese límite.};
  \node[anchor=north west,inner sep=0pt,text width=14.5cm,align=left,
    font=\fontsize{9}{11}\selectfont]
    at ([xshift=0.70cm,yshift=-5.96cm]current page.north west)
    {\textbf{Para resolver:} condiciones de contorno y continuidad de temperatura y flujo en las interfaces.};
  \node[anchor=north west,fill=darkblue,rounded corners=4pt,
    inner sep=7pt,text width=14.03cm,align=center,text=white,
    font=\fontsize{12}{14}\selectfont]
    at ([xshift=0.70cm,yshift=-6.60cm]current page.north west)
    {$I\;\longrightarrow\;Q\;\longrightarrow\;T(x,y)\;\longrightarrow\;\Tmax\;\longleftrightarrow\;T_{\mathrm{lim}}$};
  \node[anchor=north west,inner sep=0pt,text width=14.5cm,align=center,
    font=\fontsize{7.5}{9}\selectfont,text=textgray]
    at ([xshift=0.70cm,yshift=-7.63cm]current page.north west)
    {Para resistencia fija, las pérdidas Joule crecen con $I^2$. El entorno térmico condiciona la corriente admisible.};
\end{tikzpicture}
\source[\fontsize{6.0}{7.0}\selectfont]{Informe rev1: ecuación de calor, interfaces y ampacidad; IEC 60287 (2023); Enescu et al. (2021).}
\note{La diapositiva resume el marco teórico del informe rev1: fórmulas calor-estacionario-marco, interfaz y ampacidad. El régimen es estacionario, por eso no aparece la derivada temporal. La forma divergente conserva k espacialmente variable; se aplica por material y se acopla mediante continuidad de T y flujo normal. Q es una fuente volumétrica, no una potencia total: las pérdidas eléctricas del caso se distribuyen en las regiones donde se generan. Para una corriente dada se resuelve el campo y se busca la máxima temperatura dentro del conductor. Después se ajusta la corriente, por ejemplo mediante bisección, hasta Tmax=Tlim dentro de una tolerancia. La heterogeneidad cambia la disipación, por lo que cambia también la ampacidad. El límite térmico se toma del cable y del caso documentado; no se fija aquí un valor universal. Tiempo sugerido: 35--45 segundos.}
\end{frame}
...
\node[pg,fill=lightblue,draw=midblue,minimum height=1.20cm] (g1) at (0,1.68)
  {En el arreglo de cables--entorno térmico, la heterogeneidad espacial del suelo nativo, el lecho de asiento y el suelo de relleno puede modificar $T(x,y)$, $\Tmax$ e $\Imax$ respecto de una representación homogénea equivalente. Sin embargo, resulta difícil cuantificar esas diferencias de forma reproducible y establecer cuándo son relevantes para las decisiones de diseño y operación.};
...
\node[ins] (i1) at (-4.78,2.18) {Paquete de especificación física, funcional y de calidad};
\node[ins] (i2) at (0,2.18) {Paquete de referencias analíticas, manufacturadas, FEM y publicadas};
\node[ins] (i3) at (4.78,2.18) {Objetos de escenario térmico homogéneo y heterogéneo};
...
% 17 ------------------------------------------------------------------------
\begin{frame}{Problema específico 4 y articulación}
\vspace{-0.014\paperheight}
\centering
\begin{tikzpicture}[>=Latex,
  ...
  card/.style={rounded corners=5pt,draw,text width=6.12cm,minimum height=2.10cm,
    align=left,inner sep=4pt,font=\fontsize{7.35}{8.35}\selectfont},
  mini/.style={rounded corners=3pt,draw,text width=2.08cm,minimum height=0.72cm,
    align=center,inner sep=1.6pt,font=\fontsize{5.55}{6.25}\selectfont\bfseries},
...
\node[ins] (i4) at (-3.48,2.52) {Categorías de representación: heterogénea 2D y homogéneas de comparación};
\node[ins] (idsr) at (3.48,2.52) {Evidencia obtenida después de construir y evaluar el artefacto};
\node[card,fill=softorange,draw=orange] (pe4) at (-3.48,0.95)
...
\node[card,fill=softred,draw=unired] (dsr) at (3.48,0.95)
  {\textbf{Actividad metodológica posterior de DSR.} Sistematizar la evidencia, documentar el procedimiento reproducible y derivar principios de diseño. Es una tarea de síntesis y comunicación, no un problema específico adicional.};
\draw[a] (i4)--(pe4); \draw[a] (idsr)--(dsr);

\node[font=\fontsize{7.7}{8.7}\selectfont\bfseries,text=darkblue] at (0,-0.45)
  {Articulación de los cuatro problemas específicos};
\node[mini,fill=warmgray] (m1) at (-5.20,-1.32) {PE1\\Representación};
\node[mini,fill=lightblue,draw=midblue] (m2) at (-2.60,-1.32) {PE2\\Referencias};
\node[mini,fill=softgreen,draw=teal] (m3) at (0,-1.32) {PE3\\Escenarios};
\node[mini,fill=softorange,draw=orange] (m4) at (2.60,-1.32) {PE4\\Ampacidad};
\node[mini,fill=lightblue,draw=darkblue] (res) at (5.20,-1.32) {Resultado lógico\\objetivos};
\draw[a] (m1)--node[d,above]{D1}(m2); \draw[a] (m2)--node[d,above]{D2}(m3);
\draw[a] (m3)--node[d,above]{D3}(m4); \draw[a] (m4)--(res);
\node[rounded corners=4pt,draw=textgray!55,fill=warmgray,text width=13.20cm,
  ...
  {La cadena del problema termina en PE4; la síntesis reproducible se ubica después, dentro del proceso metodológico DSR.};
\end{tikzpicture}
...
\note{Presentar PE4 y recorrer la cadena de cuatro problemas. Distinguir expresamente la formulación del problema de la actividad posterior de síntesis y comunicación DSR.}
\end{frame}
...
\node[b,fill=lightblue,draw=midblue] (i) at (-4.70,0.15)
  {\textbf{Entradas discretas VI1--VI4}\\$P_1$ · $\{\mathrm{VD1},R_2\}$ · $\{\mathrm{VD2},E_3\}$ · $\{\mathrm{VD3},C_4\}$};
...
\node[b,fill=softorange,draw=orange] (r) at (4.70,0.15)
  {\textbf{Productos discretos VD1--VD4}\\Especificación · artefacto verificado · expediente térmico · expediente de ampacidad};
...
\note{Leer literalmente el objetivo general. Explicar que VI y VD son objetos discretos; VD1--VD3 se heredan en la VI siguiente. Aclarar que k(x,y) es un parámetro físico contenido en los casos.}
\end{frame}

% 19 ------------------------------------------------------------------------
\begin{frame}{Objetivos OE1--OE3}
\vspace{-0.014\paperheight}
\centering
\begin{tikzpicture}[>=Latex,
  ins/.style={rounded corners=3pt,draw=textgray!45,fill=white,text width=3.95cm,
...
\node[ins] (i1) at (-4.78,2.48) {\textbf{VI1: entrada inicial $P_1$}\\básica · completa · completa documentada};
\node[ins] (i2) at (0,2.48) {\textbf{Entrada nueva $R_2$ de VI2}\\analítica/manufacturada · FEM · publicada · mixta};
\node[ins] (i3) at (4.78,2.48) {\textbf{Entrada nueva $E_3$ de VI3}\\base · estratificada · localizada · mejorada · seca · multimaterial};
\node[oe,fill=warmgray,draw=textgray!60] (oe1) at (-4.78,0.25)
  {\textbf{OE1.} Elaborar una especificación trazable de las entradas físicas, los requisitos funcionales y los criterios de calidad del artefacto para representar el arreglo de cables--entorno térmico, incluyendo dominios multicapa, fuentes, contornos, interfaces y $\kx$.\par\vspace{0.8mm}\textbf{VD1:} especificación trazable; estado incompleto, completo pendiente o trazable aceptado.};
\node[oe,fill=lightblue,draw=midblue] (oe2) at (0,0.25)
  {\textbf{OE2.} Construir y verificar el artefacto a partir de VD1 y de referencias analíticas/manufacturadas, FEM convergentes o publicadas, clasificando su aceptación según exactitud, consistencia y robustez.\par\vspace{0.8mm}\textbf{VD2:} artefacto verificado; rechazado, aceptado con restricciones o aceptado en el dominio.};
\node[oe,fill=softgreen,draw=teal] (oe3) at (4.78,0.25)
  {\textbf{OE3.} Aplicar VD2 a un catálogo de escenarios controlados de heterogeneidad y consolidar un expediente con cambios en $\Tmax$, $T(x,y)$ y el punto caliente.\par\vspace{0.8mm}\textbf{VD3:} expediente térmico; inconcluso, sin efecto relevante, con efecto relevante no crítico o con condición crítica.};
\draw[a] (i1)--(oe1); \draw[a] (i2)--(oe2); \draw[a] (i3)--(oe3);
\draw[a] (oe1)--(oe2); \draw[a] (oe2)--(oe3);
\node[d] at (-2.39,-1.55) {VD1 $\equiv\mathrm{VI}_{2,h}$};
\node[d] at (2.39,-1.55) {VD2 $\equiv\mathrm{VI}_{3,h}$};
\end{tikzpicture}
...
\note{Cada OE es un hito. La entrada superior es el objeto nuevo; la flecha horizontal es la VD anterior, que se hereda como parte de la VI siguiente.}
\end{frame}

% 20 ------------------------------------------------------------------------
\begin{frame}{Objetivo OE4 y síntesis DSR}
\vspace{-0.014\paperheight}
\centering
\begin{tikzpicture}[>=Latex,
  ...
  card/.style={rounded corners=5pt,draw,text width=6.10cm,minimum height=2.55cm,
    align=left,inner sep=4pt,font=\fontsize{6.75}{7.75}\selectfont},
  mini/.style={rounded corners=3pt,draw,text width=2.08cm,minimum height=0.72cm,
    align=center,inner sep=1.6pt,font=\fontsize{5.45}{6.15}\selectfont\bfseries},
  a/.style={-{Stealth[length=2.1mm]},line width=0.78pt,darkblue,shorten <=1pt,shorten >=1pt},
  d/.style={font=\fontsize{5.8}{6.5}\selectfont\bfseries,fill=white,inner sep=1.1pt,text=darkblue,yshift=2pt}]
\node[ins] (i4) at (-3.48,2.75) {\textbf{VI4=$\{\mathrm{VD3},C_4\}$}\\VD3 concluyente + par homogéneo equivalente · conservador · optimista};
\node[ins] (idsr) at (3.48,2.75) {Evidencia, métricas y límites producidos por OE1--OE4};
\node[card,fill=softorange,draw=orange] (oe4) at (-3.48,0.95)
  {\textbf{OE4.} Usar VD3 y un catálogo de representaciones pareadas para elaborar un expediente comparativo de ampacidad e identificar las condiciones de mayor discrepancia.\par\vspace{0.8mm}\textbf{VD4:} expediente sin diferencia relevante, con reducción, con aumento o inconcluso/fuera del dominio.};
\node[card,fill=softred,draw=unired] (dsr) at (3.48,0.95)
  {\textbf{Actividad metodológica DSR de cierre.} Integrar los resultados del artefacto para elaborar el procedimiento, la guía de reproducción, el repositorio depurado, los límites y los principios de diseño. No constituye un objetivo específico adicional.};
\draw[a] (i4)--(oe4); \draw[a] (idsr)--(dsr);
\node[font=\fontsize{7.6}{8.6}\selectfont\bfseries,text=darkblue] at (0,-0.60)
  {Articulación de los cuatro objetivos y sus productos};
\node[mini,fill=warmgray] (m1) at (-5.20,-1.42) {OE1\\Requisitos};
\node[mini,fill=lightblue,draw=midblue] (m2) at (-2.60,-1.42) {OE2\\Verificación};
\node[mini,fill=softgreen,draw=teal] (m3) at (0,-1.42) {OE3\\Escenarios};
\node[mini,fill=softorange,draw=orange] (m4) at (2.60,-1.42) {OE4\\Ampacidad};
\node[mini,fill=lightblue,draw=darkblue] (res) at (5.20,-1.42) {Artefacto evaluado\\evidencia DSR};
\draw[a] (m1)--node[d,above]{VD1 $\equiv\mathrm{VI}_{2,h}$}(m2); \draw[a] (m2)--node[d,above]{VD2 $\equiv\mathrm{VI}_{3,h}$}(m3);
\draw[a] (m3)--node[d,above]{VD3 $\equiv\mathrm{VI}_{4,h}$}(m4); \draw[a] (m4)--node[d,above]{VD4}(res);
\node[rounded corners=4pt,draw=textgray!55,fill=warmgray,text width=13.20cm,
  minimum height=0.55cm,align=center,inner sep=2.8pt,
  font=\fontsize{6.45}{7.35}\selectfont] at (0,-2.23)
  {La sistematización comienza cuando ya existe evidencia evaluada; por ello pertenece al método DSR y no a la cadena de objetivos.};
\end{tikzpicture}
...
\note{Presentar OE4 y recorrer los cuatro objetivos. Explicar que el procedimiento y los principios se derivan en la síntesis DSR posterior, a partir de resultados ya evaluados.}
\end{frame}

\section{Variables y método}

% 21 ------------------------------------------------------------------------
\begin{frame}{Cadena discreta VI--VD y realizaciones}
\centering
\begin{tikzpicture}[
...
\node[main] (vi) at (0,2.25) {\textbf{VI y VD tienen la misma naturaleza:} paquetes documentales, artefactos versionados o expedientes discretos.\\
VD1--VD3 se conservan como la entrada heredada de la etapa siguiente.};

\node[card,fill=warmgray,draw=textgray!60] (v1) at (-4.42,0.25) {\textcolor{textgray}{\fontsize{9.0}{10.3}\selectfont\bfseries VI1 · OE1}\\[0.7mm]
\textbf{$P_1$: paquete inicial}\\básico · completo · completo documentado};
\node[card,fill=lightblue,draw=midblue] (v2) at (-1.48,0.25) {\textcolor{darkblue}{\fontsize{9.0}{10.3}\selectfont\bfseries VI2 · OE2}\\[0.7mm]
\textbf{$\{\mathrm{VD1},R_2\}$}\\VD1 aceptada + referencia analítica/manufacturada · FEM · publicada · mixta};
\node[card,fill=softgreen,draw=teal] (v3) at (1.48,0.25) {\textcolor{teal}{\fontsize{9.0}{10.3}\selectfont\bfseries VI3 · OE3}\\[0.7mm]
\textbf{$\{\mathrm{VD2},E_3\}$}\\VD2 aceptada + escenario base · estratificado · localizado · mejorado · seco · multimaterial};
\node[card,fill=softorange,draw=orange] (v4) at (4.42,0.25) {\textcolor{orange}{\fontsize{9.0}{10.3}\selectfont\bfseries VI4 · OE4}\\[0.7mm]
\textbf{$\{\mathrm{VD3},C_4\}$}\\VD3 concluyente + par homogéneo equivalente · conservador · optimista};
...
{Cada realización de VI2--VI4 contiene el producto anterior y un objeto nuevo; ninguna etapa descarta la VD que la precede.};
...
{\textbf{Cadena:} VI1 $\rightarrow$ VD1 $\equiv\mathrm{VI}_{2,h}$ · VI2 $\rightarrow$ VD2 $\equiv\mathrm{VI}_{3,h}$ · VI3 $\rightarrow$ VD3 $\equiv\mathrm{VI}_{4,h}$ · VI4 $\rightarrow$ VD4.};
\end{tikzpicture}
\source[\fontsize{6.4}{7.3}\selectfont]{Operacionalización entrada--proceso--salida del plan y articulación DSR según \textcite{peffers2007} y \textcite{gregor2013}.}
\note{Tiempo sugerido: 70 s. Explicar la composición de cada paquete, sus realizaciones y la identidad entre la VD producida y la parte heredada de la VI siguiente.}
\end{frame}

% 22 ------------------------------------------------------------------------
\begin{frame}{Parámetros, controles y metadatos}
\centering
\begin{tikzpicture}[
...
\node[card,fill=softgreen,draw=teal] at (-3.15,1.65) {{\color{teal}\fontsize{10.5}{11.8}\selectfont\bfseries Parámetros físicos del caso}\\[0.8mm]
Mapa $\kx$ · contraste $C_k$ · extensión $f_h$ · proximidad $d_h/D$ · materiales.\\[0.5mm]
Describen la física de los escenarios $E_3$; \textbf{no son VI ni VD}.};

\node[card,fill=lightblue,draw=darkblue] at (3.15,1.65) {{\color{darkblue}\fontsize{10.5}{11.8}\selectfont\bfseries Condiciones pareadas}\\[0.8mm]
Geometría y disposición · pérdidas $I^2R$ · contornos y dominio · $T_{\mathrm{amb}}$ y carga.\\[0.5mm]
Se conservan dentro de cada comparación.};

\node[card,fill=softorange,draw=orange] at (-3.15,-1.05) {{\color{orange}\fontsize{10.5}{11.8}\selectfont\bfseries Decisiones numéricas}\\[0.8mm]
Capas · neuronas · épocas · optimizador · tasa · puntos de colocación · pesos de pérdida.\\[0.5mm]
Se fijan tras el piloto; \textbf{no son VI}.};

\node[card,fill=softred,draw=unired] at (3.15,-1.05) {{\color{unired}\fontsize{10.5}{11.8}\selectfont\bfseries Metadatos registrados}\\[0.8mm]
Semilla y convergencia · tolerancia FEM · incertidumbre documental · versión del código · tiempo, memoria y hardware.\\[0.5mm]
Permiten interpretar robustez y reproducibilidad.};
...
{\textbf{Regla de separación:} VI/VD = objetos discretos de la cadena; parámetro = atributo físico; control = condición pareada; metadato = registro.};
\end{tikzpicture}
...
\note{Tiempo sugerido: 55 s. Distinguir las cuatro categorías metodológicas y remarcar que k(x,y), Ck, fh y dh/D permanecen como parámetros de los casos.}
\end{frame}

% 23 ------------------------------------------------------------------------
\begin{frame}{Productos discretos y estados: VD1--VD3}
\centering
\begin{tikzpicture}[
...
\node[head] at (0,2.18) {\textbf{Las VD son objetos, no sus métricas.} VD1--VD3 pasan íntegramente a la VI siguiente; los indicadores permiten asignarles un estado discreto.};

\node[tag,fill=warmgray,draw=textgray!55] at (-5.52,0.83) {VD1};
\node[body,fill=warmgray,draw=textgray!55] at (-1.82,0.83) {\textbf{Especificación trazable.} Estados: incompleta · completa pendiente de trazabilidad · trazable aceptada.};
\node[met,fill=white,draw=textgray!45] at (3.60,0.83) {Contenido: requisitos, catálogo, criterios y matriz requisito--prueba. Indicadores: cobertura, trazabilidad y consistencia.};

\node[tag,fill=lightblue,draw=midblue] at (-5.52,-0.52) {VD2};
\node[body,fill=lightblue,draw=midblue] at (-1.82,-0.52) {\textbf{Artefacto verificado.} Estados: rechazado · aceptado con restricciones · aceptado dentro del dominio.};
\node[met,fill=white,draw=midblue] at (3.60,-0.52) {Contenido: versión, pruebas, métricas y dominio. Indicadores: error, NRMSE, residuos, balance y robustez.};

\node[tag,fill=softgreen,draw=teal] at (-5.52,-1.87) {VD3};
\node[body,fill=softgreen,draw=teal] at (-1.82,-1.87) {\textbf{Expediente de evidencia térmica.} Estados: inconcluso · sin efecto relevante · efecto relevante no crítico · condición crítica.};
\node[met,fill=white,draw=teal] at (3.60,-1.87) {Contenido: mapas, diferencias y límites. Indicadores: $T(x,y)$, $\Tmax$, $\Delta\Tmax$, gradiente y punto caliente.};
\end{tikzpicture}
...
\note{Tiempo sugerido: 70 s. Distinguir para cada VD el objeto, sus estados y sus indicadores. Recalcar que solo los estados habilitados pasan al bloque siguiente.}
\end{frame}

% 24 ------------------------------------------------------------------------
\begin{frame}{Salida VD4 y criterio DSR}
\centering
\begin{tikzpicture}[
...
\node[tag,fill=softorange,draw=orange,text=orange] at (-5.40,1.15) {VD4};
\node[card,fill=softorange,draw=orange] at (-2.46,1.15) {{\color{orange}\fontsize{10.2}{11.5}\selectfont\bfseries Expediente comparativo de ampacidad}\\[1.0mm]
\textbf{Estados:} sin discrepancia relevante · reducción · aumento · inconcluso/fuera del dominio.\\[1mm]
\textbf{Indicadores contenidos:} $\Imax$, $\Delta I$, margen térmico y cobertura de pares.};
\node[tag,fill=softred,draw=unired,text=unired,font=\fontsize{7.2}{8.2}\selectfont\bfseries] at (0.65,1.15) {DSR};
\node[card,fill=softred,draw=unired] at (3.59,1.15) {{\color{unired}\fontsize{10.2}{11.5}\selectfont\bfseries Criterio metodológico transversal}\\[1.0mm]
\textbf{No es una quinta VD.} Se aplica sobre los cuatro objetos VD: productos aceptados reproducidos, cobertura documental, límites y trazabilidad.\\[1mm]
...
\note{Tiempo sugerido: 60 s. Explicar el resultado dependiente de OE4 y distinguirlo de la reproducibilidad, que se conserva como criterio metodológico transversal de DSR.}
\end{frame}
...
\node[phase,fill=warmgray,draw=textgray!55] (a6) at (5.52,0.40) {\textbf{6. Síntesis y comunicación}\\procedimiento, límites y principios};
...
\node[font=\fontsize{7.4}{8.5}\selectfont\bfseries,text=teal] at (1.05,1.70) {VI / paquete de entrada};
\node[font=\fontsize{7.4}{8.5}\selectfont\bfseries,text=orange] at (4.48,1.70) {VD / objeto y estados};

\foreach \y in {1.12,0.28,-0.56,-1.40}{
...
\node[fac] at (1.05,1.12) {\textbf{VI1=$P_1$}\\Básico · completo · documentado};
\node[res] at (4.48,1.12) {\textbf{VD1 · especificación}\\Incompleta · completa pendiente · trazable aceptada};

\node[pe] at (-4.95,0.28) {\textbf{PE2}\\Referencias no integradas};
\node[oe] at (-1.95,0.28) {\textbf{OE2}\\Construir y verificar};
\node[fac] at (1.05,0.28) {\textbf{VI2=$\{\mathrm{VD1},R_2\}$}\\Referencia analítica/FEM/publicada/mixta};
\node[res] at (4.48,0.28) {\textbf{VD2 · artefacto verificado}\\Rechazado · restringido · aceptado};

\node[pe] at (-4.95,-0.56) {\textbf{PE3}\\Efecto térmico no cuantificado};
\node[oe] at (-1.95,-0.56) {\textbf{OE3}\\Evaluar escenarios};
\node[fac] at (1.05,-0.56) {\textbf{VI3=$\{\mathrm{VD2},E_3\}$}\\Escenario base/estratificado/localizado/...};
\node[res] at (4.48,-0.56) {\textbf{VD3 · expediente térmico}\\Inconcluso · sin efecto · relevante · crítico};

\node[pe] at (-4.95,-1.40) {\textbf{PE4}\\Discrepancia de ampacidad desconocida};
\node[oe] at (-1.95,-1.40) {\textbf{OE4}\\Comparar ampacidad};
\node[fac] at (1.05,-1.40) {\textbf{VI4=$\{\mathrm{VD3},C_4\}$}\\Par equivalente/conservador/optimista};
\node[res] at (4.48,-1.40) {\textbf{VD4 · expediente de ampacidad}\\Sin diferencia · reducción · aumento · inconcluso};

\node[rounded corners=4pt,draw=unired,fill=softred,text width=12.25cm,minimum height=0.64cm,
  align=center,inner sep=3pt,font=\fontsize{6.55}{7.55}\selectfont] at (0,-2.30)
  {\textbf{Actividad metodológica DSR posterior:} integrar la evidencia de las cuatro filas para documentar el procedimiento reproducible, el repositorio, los límites y los principios de diseño. No agrega una quinta correspondencia problema--objetivo--hipótesis.};
\end{tikzpicture}
...
\note{Recorrer una fila y luego la herencia vertical: VD1 entra en VI2, VD2 en VI3 y VD3 en VI4. Aclarar que k(x,y), Ck, fh y dh/D son parámetros de E3, no VI ni VD.}
\end{frame}
...
{\textbf{Utilidad:} aportar evidencia para decisiones de capacidad de cables eléctricos enterrados con menor incertidumbre. El desempeño cuantitativo se establecerá durante la ejecución de Tesis 2.};
\end{tikzpicture}
