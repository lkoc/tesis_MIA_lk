"""Ejemplo tres cables XLPE 95 mm2 / 12-20 kV en formacion trefoil (toque).

Tres cables identicos enterrados en agrupacion trefoil tocante, con el
centroide del grupo a 70 cm de profundidad.
...
Archivos de datos requeridos (directorio examples/xlpe_trefoil/data/):
  ...
  cables_placement.csv      -- posicion de cada cable en la seccion
  ...
  physics_params.csv        -- parametros R(T) y k(x,y) sigmoide [NUEVO]
...
def _multilayer_T_multi(
    ...
    """Temperatura analitica para N cables identicos: superposicion Kennelly + multicapa.

    Para cada punto del dominio:
    - Suelo (fuera de todos los cables): T = T_amb + suma de Kennelly de cada cable.
    - Interior de cable i: perfil cilindrico 1D con la BC exterior basada en
      la contribucion al borde de la vaina de TODOS los cables (self + mutuo).

    La superposicion de Kennelly es exactamente T_amb en y=0 (superficie).
    La temperatura en la vaina exterior de cada cable (usada como BC para el
    perfil interior) se calcula de manera analitica escalar, asumiendo que
    la contribucion mutua se evalua en el centro del cable vecino.

    Args:
        k_soil: conductividad representativa del suelo [W/(m·K)].
                Para suelo variable, pasar k en el centroide del grupo.
    ...
    # ------------------------------------------------------------------
    # T_sheath_outer_i (escalar): temperatura analitica en la vaina
    # exterior de cada cable i = suma de contribuciones Kennelly de todos
    # ------------------------------------------------------------------
    T_sheath_outers: list[float] = []
    for i, pl_i in enumerate(placements):
        d_i = abs(pl_i.cy)
        # Self: formula exacta de Kennelly en r = r_sheath
        T_s_i = T_amb + Q_lin_each / (2.0 * math.pi * k_soil) * math.log(2.0 * d_i / r_sheath)
        # Mutuo: contribucion de cable j evaluada en el centro de cable i
        for j, pl_j in enumerate(placements):
            if i == j:
                continue
            d_j = abs(pl_j.cy)
            dist_ij = math.sqrt((pl_i.cx - pl_j.cx) ** 2 + (pl_i.cy - pl_j.cy) ** 2)
            # r_img_j_at_i = distancia de la imagen de cable j al centro de cable i
            r_img_sq = (pl_i.cx - pl_j.cx) ** 2 + (pl_i.cy - d_j) ** 2
            T_s_i += Q_lin_each / (4.0 * math.pi * k_soil) * math.log(r_img_sq / dist_ij ** 2)
    ...
    for i in range(len(placements)):
        ...
        for layer in reversed(layers):
            ...
            if layer.r_inner == 0.0 and Q_lin_each > 0.0:
                Q_vol = Q_lin_each / (math.pi * r_out ** 2)
                T_curr += Q_vol / (4.0 * layer.k) * r_out ** 2
            else:
                T_curr += Q_lin_each / (2.0 * math.pi * layer.k) * math.log(r_out / r_in)
        layer_T_outer_list.append(layer_T_outer)

    # ------------------------------------------------------------------
    # Distancias radiales por cable (reutilizadas para suelo e interior)
    # ------------------------------------------------------------------
    ...
    for r_i in r_per_cable:
        soil_mask &= (r_i.squeeze(1) >= r_sheath)
    ...
    for pl in placements:
        dx     = xy[:, 0:1] - pl.cx
        d_pl   = abs(pl.cy)
        dy_img = xy[:, 1:2] - d_pl          # y - (+d)  [imagen en y=+d]
        dy_r   = xy[:, 1:2] - pl.cy         # y - cy
        r_img_sq = (dx * dx + dy_img * dy_img).clamp(min=1e-20)
        r_sq     = (dx * dx + dy_r  * dy_r).clamp(min=r_sheath ** 2)
        dT_soil += Q_lin_each / (4.0 * math.pi * k_soil) * torch.log(r_img_sq / r_sq)
    ...
    for i, pl in enumerate(placements):
        ...
        cable_mask = r.squeeze(1) < r_sheath
        ...
        for layer in reversed(layers):
            ...
            if layer.r_inner == 0.0 and Q_lin_each > 0.0:
                ...
                dT = Q_lin_each / (2.0 * math.pi * layer.k) * torch.log(
...
def _pretrain_trefoil(
    ...
    """Pre-entrenar en: interiores de los 3 cables (T analitica) + 4 bordes (T_amb).
    ...
    for pl in placements:
        ...
        T_c    = _multilayer_T_multi(xy_c, layers, placements, k_soil, T_amb, Q_lin_each)
...
class ResidualPINNModelMulti(nn.Module):
    ...
    Nota: _Q_lin es mutable (no frozen) para soportar la iteracion R(T).
    """
    ...
    def __init__(
        ...
        self._k_soil = k_soil
        self._T_amb = T_amb
        self._Q_lin = Q_lin_each   # mutable: se actualiza en iteracion R(T)
...
def _iec60287_trefoil(
    ...
    q_lin_override: float | None = None,
) -> dict:
    """Estimacion IEC 60287 para N cables en trefoil: resistencias en serie + calentamiento mutuo.

    Args:
        k_eff_fn: si se pasa, k(x,y) escalar para la estimacion Kennelly (suelo variable).
                  Si None, usa k_soil uniforme.
        q_lin_override: si se pasa, sobreescribe el Q_lin calculado desde cable_layers.csv.
                        Util para la iteracion R(T).
    """
    conductor = layers[0]
    r_cond    = conductor.r_outer
    if q_lin_override is not None:
        Q_lin = q_lin_override
    else:
        Q_lin = conductor.Q * Q_scale * math.pi * r_cond ** 2   # W/m lineal
    r_sheath  = layers[-1].r_outer

    # Resistencia termica de las capas del cable (identica para todos)
    ...
    dT_cable_total = sum(dT_layers.values())
    ...
    for i, pl_i in enumerate(placements):
        ...
        # Self: formula de Kennelly en vaina
        dT_soil_i = Q_lin / (2.0 * math.pi * k_i) * math.log(2.0 * d_i / r_sheath)
        # Mutuo: cable j evaluado en el centro de cable i
        for j, pl_j in enumerate(placements):
            if i == j:
                continue
            d_j      = abs(pl_j.cy)
            dist_ij  = math.sqrt((pl_i.cx - pl_j.cx) ** 2 + (pl_i.cy - pl_j.cy) ** 2)
            r_img_sq = (pl_i.cx - pl_j.cx) ** 2 + (pl_i.cy - d_j) ** 2
            dT_soil_i += Q_lin / (4.0 * math.pi * k_i) * math.log(r_img_sq / dist_ij ** 2)
        T_cond_i = T_amb + dT_soil_i + dT_cable_total
...
def _train_adam_lbfgs(
    ...
    for step in range(1, adam_steps + 1):
        if xy_soil is None or (step - 1) % print_every == 0:
            xy_soil = _sample_soil_pts(domain, placements, r_sheath, n_int, device, oversample)
    ...
    if lbfgs_steps > 0:
        ...
        for event in range(1, n_events + 1):
            xy_s = _sample_soil_pts(domain, placements, r_sheath, n_int, device, oversample)
...
def _plot_zoom_temperature(
    ...
    layer_names  = [la.name for la in reversed(layers)]
    for pl in placements:
...
def _plot_geometry_trefoil(
    ...
    """Visualizar dominio con los tres cables del trefoil (seccion transversal)."""
    fig, ax = plt.subplots(figsize=(10, 6))
    ...
    for idx, (pl, color) in enumerate(zip(placements, colores_cable)):
        ...
        for li, (layer, lcol) in enumerate(zip(reversed(layers), reversed(layer_colors))):
            ring = plt.Circle((pl.cx, pl.cy), layer.r_outer, color=lcol, zorder=5)
            ax.add_patch(ring)
        ax.text(
            pl.cx, pl.cy + layers[-1].r_outer * 1.8,
...
def main() -> None:
    ...
    SEP = "=" * 66
    print(SEP)
    print("  PINN -- Tres cables XLPE 95 mm2 / Trefoil tocante")
    ...
    if use_R_T:
        q_lin_iec = Q_lin_from_I(pp.I_A, pp.R_ref, pp.alpha_R, T_amb, pp.T_ref_R_K)
        T_cond_iec_est = T_amb + 50.0
        for _ in range(15):
            q_lin_iec = Q_lin_from_I(
                pp.I_A, pp.R_ref, pp.alpha_R, T_cond_iec_est, pp.T_ref_R_K
            )
            iec_tmp = _iec60287_trefoil(
                problem.layers, problem.placements, k_soil, T_amb, scenario.Q_scale,
                k_eff_fn=k_eff_fn_iec, q_lin_override=q_lin_iec,
            )
        ...
        q_lin_iec = problem.layers[0].Q * scenario.Q_scale * math.pi * problem.layers[0].r_outer ** 2
        iec = _iec60287_trefoil(
            problem.layers, problem.placements, k_soil, T_amb, scenario.Q_scale,
            k_eff_fn=k_eff_fn_iec,
        )
    T_ref_K = iec["T_cond_ref"]
    q_lin   = iec["Q_lin_W_per_m"]   # Q_lin inicial para el PINN
    ...
    print("  R(T)        : %s" % ("SI  (I=%.0f A, R_ref=%.4e Ohm/m, alpha=%.5f /K)" % (
        pp.I_A, pp.R_ref, pp.alpha_R) if use_R_T else "No"))
    if use_R_T:
    ...
    print("  Q_lin por cable : %.2f W/m lineal" % iec["Q_lin_W_per_m"])
    ...
    r_sheath = problem.layers[-1].r_outer
    ...
    _plot_geometry_trefoil(
        problem.layers, problem.placements, problem.domain,
        title="Tres cables XLPE 95 mm2 -- Trefoil tocante (detalle)",
        save_path=RESULTS_DIR / "geometry.png",
    )

    # Grafica campo k(x,y)
    _plot_k_field(
        problem.domain, pp, problem.placements, problem.layers,
    ...
    # Pre-entrenamiento
    # ------------------------------------------------------------------
    print("\n  Pre-entrenando en perfiles cilindricos de los 3 cables (800 pasos)...", flush=True)
    rmse_pre = _pretrain_trefoil(
        model, problem.placements, problem.domain, problem.layers,
        q_lin, k_eff_bg, T_amb, device=device, normalize=normalize,
    ...
    for iter_i in range(n_iters):
        ...
        if use_R_T and n_iters > 1:
            print("\n  -- R(T) iteracion %d/%d -- Q_lin=%.3f W/m" % (
                iter_i + 1, n_iters, model._Q_lin))
        ...
        # Actualizar Q_lin para la proxima iteracion
        if use_R_T and not is_last:
            T_cond_hottest = max(T_cond_pinns)
            q_lin_new = Q_lin_from_I(
                pp.I_A, pp.R_ref, pp.alpha_R, T_cond_hottest, pp.T_ref_R_K
            )
            model._Q_lin = q_lin_new
            print("  --> T_cond estimada = %.1f K (%.1f degC)  =>  Q_lin nuevo = %.3f W/m" % (
                T_cond_hottest, T_cond_hottest - 273.15, q_lin_new,
            ))
    ...
    plot_temperature_field(
        X, Y, T_grid,
        title="T(x,y) [K] -- Tres cables XLPE 95 mm2 Trefoil  [%s]" % profile,
        save_path=RESULTS_DIR / "temperature_field.png",
    )
    _plot_zoom_temperature(
        model, problem.domain, problem.placements, problem.layers,
    ...
    print("=" * 66)
    if use_R_T:
        q_lin_final = model._Q_lin
        R_final = q_lin_final / (pp.I_A ** 2) if pp.I_A > 0 else 0.0
        T_cond_hottest_final = max(T_cond_pinns)
        print("  Q_lin final (R(T))  : %.3f W/m  (R=%.6f Ohm/m)" % (q_lin_final, R_final))
        print("  Incremento Q (R(T)) : %+.1f%%  vs. Q fria" % (
            100.0 * (q_lin_final / iec["Q_lin_W_per_m"] - 1.0)))
