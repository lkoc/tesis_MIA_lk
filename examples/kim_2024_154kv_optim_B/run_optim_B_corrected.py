...
Changes vs run_optim_B.py:
  ...
  - Header updated; BEST_KNOWN placeholder left empty (to fill after search)
...
Usage::
    ...
    # Fast exploration (30 trials, ~3 min each)
    python examples/kim_2024_154kv_optim_B/run_optim_B_corrected.py --fast --trials 30

    # Full runs once best is known
    python examples/kim_2024_154kv_optim_B/run_optim_B_corrected.py --trials 30

    # Re-run best config across 5 seeds, save model
    python examples/kim_2024_154kv_optim_B/run_optim_B_corrected.py --best --n-seeds 5
"""
...
def run_trial(
    ...
    # Pre-train with corrected boundary temperatures
    pretrain_multicable(
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="HP search for Kim 2024 Case B (quick 64×4 PINN) — CORRECTED T_amb")
...
# Best config found under corrected T_amb=300.30 K.
# From 30-trial search: T=70.14°C, Error=-0.46K (BEST)
...
def main() -> None:
    ...
    print("  [CORRECTED: T_amb=300.30 K, bc_temps fix applied]")
    print("=" * 72)
    ...
    print()
    print("  Next: update BEST_KNOWN with these params, then run:")
    print("    python run_optim_B_corrected.py --best --n-seeds 5")
