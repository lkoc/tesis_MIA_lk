"""Re-run best 64×4 config with corrected T_amb = 300.30 K.

T_amb correction rationale
--------------------------
The Kennelly background uses T_amb as the isothermal surface temperature (y=0)
for the method of images.  The original value (290.15 K) was the ambient air
temperature from scenarios.csv, but the actual FEM surface temperature under
the Robin BC (h=7.371 W/(m²K), T_air=300.35 K, T_bot=288.35 K) is:

    R_soil = 0.56/1.804 + 1.2/1.351 + 43.74/1.517 = 30.03 m²K/W
    T_surf = (h·T_air + T_bot/R_soil) / (h + 1/R_soil) = 300.30 K

Using T_amb=290.15 forced the NN to learn a +9.75 K correction in the cable
zone; with T_amb=300.30 that correction is ~0 K, eliminating the systematic
~2 K bias observed in the cable zone.

Prerequisites
-------------
scenarios.csv must have T_amb=300.30 (already updated):
    summer_pac,steady,1.0,1.55,300.30,0

Usage::
    python examples/kim_2024_154kv_optim_B/run_corrected_64x4.py
    python examples/kim_2024_154kv_optim_B/run_corrected_64x4.py --n-seeds 8
"""
...
# Constants
# ---------------------------------------------------------------------------
PAPER_T_MAX_PAC  = 273.15 + 70.6   # FEM reference
PAPER_W_D        = 3.57
PAPER_FREQ       = 60.0
FEM_REF_C        = 70.6
T_AMB_CORRECTED  = 300.30           # K — FEM surface temp from 1D Robin BC analysis

# ---------------------------------------------------------------------------
# Best known config for 64×4 (from run_optim_B.py search)
# ---------------------------------------------------------------------------
BEST_64x4 = dict(
    lr               = 1e-3,
    adam_steps       = 2000,
...
def run_trial(p: dict, shared: dict, device: torch.device, seed: int = 42) -> dict:
    ...
    # Per-edge pretrain BC temps: top=T_amb (Robin BC result), bottom/sides=actual Dirichlet
    # This avoids the large pretrain mismatch (≈-13 K) at the cold bottom isotherm that
    # destabilises early training when T_amb=300.30 K ≫ T_bc_bottom=288.35 K.
    _bc_pretrain = {
        "top":    T_amb,   # Robin BC → T_surface ≈ T_amb = 300.30 K
        "bottom": problem.bcs["bottom"].value,  # Dirichlet 288.35 K
...
def _parse_args():
    ap = argparse.ArgumentParser(
        description="Multi-seed best 64×4 run with corrected T_amb=300.30 K")
...
def main():
    ...
    print("=" * 72)
    print("  CORRECTED T_amb — 64×4 model (small network)")
    print("  T_amb = 300.30 K  (FEM surface temp, corrected from 290.15 K)")
    print("  Bias fix: NN learns ~0 K correction near cables (was +9.75 K)")
    ...
    model_path = RESULTS_DIR / "model_best_64x4.pt"
    torch.save({
    ...
    print()
    print("  Next: run  python examples/kim_2024_154kv_optim_C/run_corrected_128x5.py")
