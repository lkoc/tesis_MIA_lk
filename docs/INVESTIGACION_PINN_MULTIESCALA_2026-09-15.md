...
### 9.4. Diseño recomendado para la extensión no lineal y temporal (sin ejecutar)

La solución SVD demuestra que conviene distinguir el problema físico del algoritmo de entrenamiento. Para este problema lineal, resolver una base neuronal por mínimos cuadrados puede ser más eficiente que optimizar todos los pesos. La PDE sigue evaluándose en todos los puntos interiores; no se introduce FEM en el entrenamiento, ni una relación térmica resistiva entre capas.

Si k es independiente de T y Q=σ(T)Ez², la parte conductiva conserva una matriz fija para una geometría/base dadas. Un algoritmo candidato sería:

1. Evaluar T anterior en colocaciones y en una cuadratura independiente del conductor; calcular σ y Ez=I/∫σ dA.
2. Recalcular Q(x,y) en el conductor, sin sustituirlo por una temperatura media ni por una fuente superficial.
3. Resolver el sistema neuronal con ese Q; aplicar relajación si fuese necesaria.
4. Exigir convergencia tanto del campo y la corriente/potencia como del **residuo no lineal recomputado** con T final. Comparar con FEM de la misma física.

Es un esquema de Picard propuesto, no una prueba de convergencia. Puede requerir Newton o continuación en corriente cerca de límites térmicos. Si k depende de T también cambia la matriz; si hay efecto piel, la fuente debe usar la distribución y procedencia eléctrica documentadas.

Para transitorio se puede añadir tiempo a las redes y a las trazas, o usar una base espacial neuronal y una integración temporal implícita de ρcTt−div(k∇T)=Q. Esta última opción debe denominarse discretización temporal de una aproximación neuronal de la PDE, no una PINN espacio-temporal entrenada si no lo es. Las interfaces continúan compartiendo T y q a cada tiempo. Se requieren capacidades por material, condición inicial compatible y convergencia temporal además de espacial. La escala difusiva local h²ρc/k puede ser muy distinta entre capas; ventanas temporales y pasos adaptativos deben justificarse con esos tiempos, sin desincronizar materiales acoplados.

### 9.5. Condiciones para aceptar la propuesta como resultado de tesis

La construcción circular y dos semillas de un caso no bastan para afirmar «mejor PINN para cables». Deben separarse estas afirmaciones:

- **Demostrada por la construcción:** continuidad de T y q en interfaces circulares del prototipo, regularidad del núcleo, preservación de la difusión angular.
- **Medida en el piloto:** error FEM, coste observado y aceptación para una geometría, coeficientes y semillas declarados.
- **Pendiente:** robustez con estratos, varios cables, espesores nuevos, no linealidad, tiempo y parámetros variables; superioridad en tiempos aislados a igualdad de precisión.

La selección propuesta tiene dos niveles: locELM/PIELM con cartas locales y trazas duras como candidata para el problema lineal; modelo por subdominios con proyección variable/Newton o formulación variacional para la extensión no lineal. No hay fundamento para exigir que el mismo optimizador sea óptimo en todas las etapas de la tesis.

### 9.6. Dónde mejorar eficiencia y colocación sin multiplicar todo por diez

La auditoría del sistema lineal de los dos pilotos encuentra condición 2 de aproximadamente 2,16e13 y 2,46e13 antes de escalar columnas; después, 1,47e8 y 1,96e8. Sigue siendo un sistema delicado: se justifica float64 y resolver por SVD/QR, evitando ecuaciones normales AᵀA que agravan el condicionamiento. La normalización de columnas mejora la aritmética sin cambiar el mínimo de la pérdida salvo por el umbral numérico de truncamiento, que queda registrado.

En el primer piloto, construir A consumió 53,55 s y resolverla 0,076 s. El blanco inmediato de optimización es **ensamblar las características y sus derivadas una vez por subdominio**, no acelerar una SVD que apenas pesa. El prototipo monta columnas mediante evaluaciones completas para que la afinidad sea fácil de auditar; no es una implementación final eficiente. La versión de producción debería evaluar características, gradientes y Hessianos en lotes, reutilizar geometría, k y máscaras, y aprovechar la estructura dispersa por material.

Para varios cables, cada bloque interior comparte pocos coeficientes de traza con sus vecinos. Es posible eliminar coeficientes interiores por álgebra lineal local y resolver el acoplamiento global, reconstruyendo después los campos 2D. Esa condensación algebraica **no sustituye las capas por resistencias térmicas**: se mantienen sus funciones espaciales, PDE y grados de libertad. La propuesta de implementación debe compararse con el ensamblado monolítico antes de afirmar escalabilidad.

Una mejora adicional de colocaciones, propuesta y aún no ensayada, es combinar:

- cobertura obligatoria por material y por banda de distancia;
- residuo PDE adimensional sobre candidatos independientes;
- información de independencia de filas de la base neuronal, mediante puntuaciones de influencia (*leverage*) calculadas con QR/SVD, no con la inversa explícita de AᵀA;
- balances en pequeños volúmenes alrededor de interfaces y estimaciones locales del error en flujo.

El residuo identifica dónde la solución actual incumple la PDE; la información de las filas identifica qué puntos agregan restricciones distintas. Son criterios complementarios. Se conservaría exploración uniforme y se contabilizaría el coste de valorar candidatos. Debe compararse con muestreo residual simple a **igual tiempo total**, con pesos de integración/importance sampling documentados. Esta combinación es una hipótesis de mejora, no un resultado medido en los ensayos actuales.

El refinamiento ejecutado cambia también cuadraturas de interfaz y frontera exterior. Por tanto, sus cambios son sensibilidad conjunta de entrenamiento e integración de balances; no permiten atribuir toda mejora a la cantidad de colocaciones interiores.

### 9.7. Reproducción y precisión del tamaño del modelo

Se repitió el piloto SVD de semilla 11 en un paquete aislado reconstruido con **las fuentes archivadas de esa ejecución**, verificadas mediante SHA256. La matriz A, el vector b y el campo de temperatura coinciden bit a bit; diferencia máxima 0 K. La aceptación se mantiene. Evidencia: `Benchmarks/multiscale_exploration/reproduction/audit.json`. Es una repetición de control, excluida del conteo de ensayos de selección.

Los 269 coeficientes ajustados de 32×3 son 104 de trazas (cuatro interfaces, temperatura/flujo, trece términos reales por traza) y 165 de salidas neuronales. El objeto almacenado contiene 11472 parámetros, incluidos pesos fijos y algunos parámetros heredados inactivos. No se debe presentar como una red de sólo 269 pesos totales. La reducción 16×2 ajusta 189 coeficientes, pero falla la semilla 23 por error térmico; no se elige por ser menor.

Tras la consulta inicial del resumen de Chung et al., se pudo acceder al [PDF v2](https://arxiv.org/pdf/2604.08453v2) y revisar su introducción y planteamiento. La advertencia sobre restricciones multiplicativas, geometría y libertad de aproximación refuerza la necesidad de ensayar los collares y las esquinas: nuestra verificación circular no resuelve por sí sola esa extensión. También se accedió al artículo completo de locELM del repositorio de Purdue. La propuesta se mantiene como adaptación y combinación verificable, sin afirmación de prioridad sobre estos antecedentes.

### 9.8. Prueba de transferencia exploratoria registrada

Tras completar los ocho ajustes SVD y mientras terminan los dos pilotos Adam/L-BFGS, se fija una comprobación adicional sin retocar la receta: 32×3, seis armónicos, 512/256/192 puntos, semillas 11 y 23, SVD y umbral idénticos, en `coaxial_angular` y `xlpe_dry_near`. Se reutilizan sus FEM convergidos de igual física; no se usa información FEM para entrenar. Son cuatro nuevas soluciones exploratorias.

El primer caso fuerza dependencia angular mediante la frontera exterior; el segundo mantiene el dominio 8×4 m y añade heterogeneidad continua del suelo. Ambos caben en la geometría circular de un solo cable del prototipo. No se denominarán validación ciega: sus problemas ya figuraban en la campaña anterior. Esta comprobación delimita si el éxito inicial se extiende fuera de `xlpe_single`; cualquier fallo se reportará sin optimizar después sobre estas mismas observaciones.

### 9.9. Resultado de transferencia y nueva iteración de desarrollo registrada

`coaxial_angular` pasa ambas semillas: errores Tmax 0,0016 y 0,0006 K. `xlpe_dry_near` falla ambas: errores Tmax 1,2040 y 2,3140 K. Las interfaces exactas no bastan para modelar la heterogeneidad del suelo. Se conserva ese resultado de transferencia sin corregirlo ni sustituirlo.

Se abre una **iteración de desarrollo distinta**, motivada por ese fallo y registrada antes de ejecutarla. No modifica la receta de la sección 9.8 ni se presenta como confirmación. Se añade al suelo un experto 16×2 con entradas locales u=(x−xp)/wx, v=(y−yp)/wy, log(r/R) y χ=(k(x,y)−k_fondo)/(k_parche−k_fondo). χ proviene del coeficiente físico conocido, no de FEM. La corrección tiene forma E(x,y) exp[−(u²+v²)/2] Nlocal(u,v,log(r/R),χ), donde E es la envolvente Dirichlet exterior ya usada. La representación global se conserva y la corrección de trazas se aplica al campo completo; las interfaces del cable continúan exactas.

Este es un modelo global/local aditivo, inspirado por el principio multilevel, pero no una reproducción completa de FBPINNs. La zona seca de este caso es continua; incorporar χ así no es válido sin cambios en un salto discontinuo de k. No se borra ∇k en la PDE al usar k como entrada: se mantienen todas las derivadas de la regla de la cadena.

Se fijan sólo dos ejecuciones: `xlpe_dry_near`, semillas 11 y 23, mismo presupuesto de puntos 512/256/192, misma SVD y seis armónicos, sin retocar tolerancias. El bloque añadido aporta 17 coeficientes de salida: total 286 ajustados. No se optimizan aquí tamaño del parche ni amplitud gaussiana mediante FEM. La comparación es una ablación de representación a igual muestreo. Si mejora, será evidencia de desarrollo que requiere una nueva confirmación; si falla, quedará documentado el límite.

## 10. Cierre de esta exploración: resultados y dictamen

Se completaron **16 ajustes nuevos**: dos entrenamientos completos y catorce ajustes de características fijas. Se reutilizaron dos referencias PINN de desarrollo anteriores y se realizó una repetición de control con código archivado. El ensayo corto de ejecución se excluye del conteo científico. No quedaron procesos Python entrenando al cerrar esta exploración. La auditoría de cierre está en [completion_audit.json](../Benchmarks/multiscale_exploration/completion_audit.json).

| Comparación | Resultado observado | Interpretación admisible |
|---|---|---|
| PINN anterior, `xlpe_single`, mismas semillas/presupuesto | 2/2 aceptadas; error Tmax 0,306–0,336 K; aproximadamente 402 s | Referencia útil ya existente; no se repitió su coste en un ensayo aislado de hardware. |
| Trazas duras + Adam/L-BFGS, `xlpe_single` | 0/2; error Tmax 14,70–14,96 K; 1726–1739 s | Las restricciones exactas **sin resolver el condicionamiento de la optimización** no bastan. Esta variante se descarta como receta en su configuración ensayada. |
| Trazas + SVD 32×3, puntos centrales, `xlpe_single` | 2/2; error Tmax 0,035–0,103 K; 50–54 s; 269 coeficientes ajustados | Candidata eficaz para capas milimétricas en 8×4 m, bajo generación prescrita. |
| Misma arquitectura SVD, tres densidades | 6/6 aceptadas; último refinamiento <0,5% de ΔT FEM en campo y Tmax, ambas semillas | Evidencia preliminar de insensibilidad en el último refinamiento; no demuestra una tasa asintótica ni se transfiere al experto de suelo. |
| SVD 16×2, mismos puntos centrales | 1/2; error Tmax de la segunda semilla 0,917 K | No elegir la red menor usando sólo la semilla favorable. 32×3 es la menor de las dos configuraciones probadas que pasa ambas. |
| SVD 32×3, coaxial angular | 2/2; error Tmax 0,0006–0,0016 K | La representación admite variación angular; no es una solución radial 1D. |
| SVD 32×3 sin experto de suelo, zona seca cercana | 0/2; error Tmax 1,204–2,314 K | El éxito en materiales del cable no garantiza resolver heterogeneidades del suelo. |
| Misma zona seca y puntos, con experto local 16×2 | 2/2; error Tmax 0,477–0,547 K; energía máxima 0,091–0,121%; 47–69 s; 286 coeficientes | La corrección global/local mejora la representación en este caso. Se verificó identidad bit a bit de las colocaciones de la ablación. Es desarrollo posterior al fallo, no confirmación independiente. |

Las diferencias de tiempo son medidas reales de estas ejecuciones concurrentes; no prueban factores universales de aceleración ni superioridad frente a FEM. El campo SVD se compara siempre con FEM de la misma física y malla convergida. La diferencia de 0,035 K en Tmax no implica error máximo de 0,035 K en todo el dominio: en el suelo hay errores mayores, recogidos por RMSE y mapas.

**Recomendación metodológica:** adoptar como siguiente candidata una familia de aproximaciones neuronales por material, con coordenadas logradiales/cartesianas locales, trazas compartidas de T y q y representación global/local del suelo. Para la etapa lineal, ajustar salidas y trazas mediante mínimos cuadrados escalados/SVD. Empezar con 32×3 por material, seis armónicos y 512/256/192 puntos; añadir un experto 16×2 por heterogeneidad suave identificada cuando corresponda. Son valores iniciales respaldados por estos pilotos, no óptimos universales. Las zonas de k discontinuo requieren separación material explícita; no deben suavizarse sólo para facilitar el entrenamiento.

El modelo es único como composición acoplada, pero sus coeficientes se resuelven **por caso**. No se ha entrenado un operador o una PINN paramétrica que generalice sin nuevo ajuste a cualquier geometría, corriente y material. La ampliación a varios cables y estratos necesita nuevas verificaciones de compatibilidad y cobertura; para Q(T) y transitorio se requiere la iteración/temporalización descrita, con referencias FEM correspondientes.

El aporte creativo que sí sostiene esta exploración es la **combinación comprobada** de transmisión exacta, cartas de escala, solución lineal de la base neuronal y enriquecimiento local guiado por la heterogeneidad física. La originalidad absoluta y la robustez en todo el alcance de la tesis siguen sin establecerse. No se recomienda volver a multiplicar puntos indiscriminadamente ni repetir toda la producción antes de resolver esa ampliación.

### Evidencia y siguiente etapa

- [Resultados, tamaños y sensibilidad a puntos](../Benchmarks/multiscale_exploration/report/RESULTADOS.md).
- [Transferencia sin reajuste, incluidos los fallos](../Benchmarks/multiscale_exploration/report/TRANSFERENCIA.md).
- [Experto local: iteración de desarrollo](../Benchmarks/multiscale_exploration/report/EXPERTO_LOCAL.md).
- [Mapa FEM/PINN/error en suelo y capas](../Benchmarks/multiscale_exploration/report/campos_semilla11.png).
- [Geometría y escalas](../Benchmarks/multiscale_exploration/report/escalas.png).
- [Cómo repetir y qué archivos conservar](../Benchmarks/multiscale_exploration/README.md).

La próxima campaña debe fijar, antes de ejecutarse: geometrías nuevas y semillas nuevas de confirmación; número de expertos y armónicos; convergencia de colocaciones **de la receta global/local final**; ensayo independiente de puntos interiores frente a cuadratura; extensión a interfaces planas y múltiples cables; comparación de coste aislada. Sólo después procede integrar la metodología seleccionada y sus resultados confirmados en todos los capítulos.

**Estado del proceso anterior:** siguen pendientes la confirmación PINN de ampacidad y el cierre integral de la tesis. Esta investigación no los declara terminados ni traslada resultados exploratorios a sus conclusiones. El documento de tesis no se actualizó con esta nueva propuesta durante la exploración, conforme a la separación solicitada.
