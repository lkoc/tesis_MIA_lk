...
\textcite[15--16]{atoccsa2024} & Tres cables XLPE de \SI{220}{\kilo\volt} & Resistividad de \SI{2.5}{\kelvin\metre\per\watt} frente a \SI{3.0}{\kelvin\metre\per\watt} & La ampacidad disminuyó de \SI{1157}{\ampere} a \SI{1064}{\ampere}. \\
...
La justificación tecnológica corresponde a la construcción de un artefacto que combine conocimiento físico y aprendizaje automático. La PINN incorpora la PDE dentro de su función de pérdida y puede aproximar un campo continuo sin depender exclusivamente de pares etiquetados para todos los puntos del dominio \parencites[686--690]{raissi2019}[1--3,12]{aldulaimi2024}.
...
Los resultados no podrán generalizarse fuera de las geometrías, materiales, fuentes, contornos y rangos verificados. Cualquier comparación de recursos computacionales deberá informar hardware, tolerancias, semillas e inicializaciones.
...
La tesis profesionalizante no plantea una contrastación estadística de hipótesis. Las proposiciones formuladas durante el plan se conservan como criterios previos de calidad, utilidad y efecto físico en la metodología y en el Capítulo~\ref{ch:propuesta-desarrollo}. Esta adaptación evita presentar como demostrado un resultado que todavía debe evaluarse.
...
\label{for:metricas}
\begin{aligned}
e_{T_{\max}}(\pct)&=100\frac{\left|T_{\max}^{\mathrm{PINN}}-T_{\max}^{\mathrm{ref}}\right|}{\left|T_{\max}^{\mathrm{ref}}\right|},\\
...
La primera expresión mide la diferencia relativa de temperatura máxima frente a una referencia. La segunda mide el cambio de ampacidad entre el escenario heterogéneo y su par homogéneo. No constituyen por sí mismas evidencia de validez; deben interpretarse con residuos, balance, convergencia y límites del caso.
...
\pendiente{Cerrar la metodología ejecutada después del piloto: especificar casos definitivos, niveles de contraste, extensión y proximidad; arquitectura e hiperparámetros; número de semillas; hardware; versiones de software; referencia FEM; reglas finales de convergencia; y cualquier ajuste previo a los criterios de aceptación.}

En síntesis, el capítulo establece el problema, los objetivos y el protocolo que debe producir evidencia. El marco teórico del Capítulo~\ref{ch:marco-teorico} desarrolla las bases físicas, normativas, numéricas y metodológicas necesarias para interpretar esa evidencia.
