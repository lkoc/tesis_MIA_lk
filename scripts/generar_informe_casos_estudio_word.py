...
DOCX_PATH = OUT_DIR / "Informe_datos_casos_estudio_papers.docx"
...
def table(headers: list[str], rows: list[list[object]]) -> str:
    ...
    for row in rows:
        ...
        widths = [max(w, len(v)) for w, v in zip(widths, cleaned)]

    def fmt(row: list[str]) -> str:
        return "| " + " | ".join(v.ljust(w) for v, w in zip(row, widths)) + " |"

    out = [fmt(headers), "| " + " | ".join("-" * w for w in widths) + " |"]
    out.extend(fmt(row) for row in cleaned_rows)
    return "\n".join(out)
...
metadata_rows = [
    [
        "kim2025",
        "Kim et al. (2025)",
        "UPCS 154 kV con seis cables, dos capas planas, duct bank, bedding PAC/NC/arena y tres estratos de suelo natural.",
        "Cable multicapa, geometria de instalación, suelos, bedding, fronteras térmicas, temperatura máxima y capacidad.",
    ],
    [
        "khumalo2025",
        "Khumalo et al. (2025)",
        "Cable MV XLPE bajo secado de suelo; compara humedad, resistividad, profundidad, temperatura de suelo y rating.",
        "Resistividad del suelo, humedad, profundidad, temperatura ambiente del suelo, ampacidad.",
    ],
    [
        "aldulaimi2024",
        "Al-Dulaimi et al. (2024)",
        "Cable 132 kV XLPE con FEM-BPNN; arreglos plano/trébol, suelos mono/multicapa, backfill SCMB/FTB y zonas secas.",
        "Geometria 2D, configuraciones, materiales, conductividades, condiciones de frontera, Tmax.",
    ],
    [
        "atoccsa2024",
        "Atoccsa et al. (2024)",
        "Cable 220 kV XLPE con backfill térmico optimizado por PSO dinámico.",
        "Datos del cable, variables de zanja/backfill, costos, ampacidad con y sin backfill.",
    ],
    [
        "oclon2015",
        "Ocłoń et al. (2015)",
        "Sistema 400 kV en formación plana con tubos HDPE, SBM y FTB optimizado por PSO.",
        "Capas del cable, ductos, conductividades, dominio FEM, profundidad, separación, Tmax.",
    ],
    [
        "aras2005",
        "Aras et al. (2005)",
        "Cable 154 kV XLPE comparado por IEC, FEM y ensayo térmico de laboratorio.",
        "Geometria, propiedades térmicas, profundidad, dominio FEM, ampacidad/flujo térmico.",
    ],
]
...
def main() -> None:
    ...
    subprocess.run(
        [
            ...
            "--standalone",
            "--metadata",
            "title=Informe de datos de casos de estudio extraídos de papers",
        ],
        cwd=ROOT,
        check=True,
    )
