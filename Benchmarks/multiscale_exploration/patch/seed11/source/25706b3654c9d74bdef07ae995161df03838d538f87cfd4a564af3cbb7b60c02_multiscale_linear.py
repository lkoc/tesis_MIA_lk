...
def main():
    ...
    ap.add_argument('--factor', type=float, choices=[.5,1.,2.], default=1.)
    ap.add_argument('--width', type=int, default=32)
    ap.add_argument('--depth', type=int, default=3)
    ap.add_argument('--case', choices=['xlpe_single','coaxial_angular','xlpe_dry_near'], default='xlpe_single')
    ap.add_argument('--patch-expert', action='store_true')
    args = ap.parse_args()
    ...
    case = config['physics']['case'] if args.case=='xlpe_single' else json.loads((ROOT/'Benchmarks/explicit_study/cases'/f'{args.case}.json').read_text())
    domain = FullDomain(case, config['physics']['source_mode'])
    config.update(physics=domain.specification(), physics_sha256=domain.fingerprint, case=args.case)
    config['reference'] = str(ROOT/'Benchmarks/explicit_study/references/fixed'/args.case/'fem_l2.npz')
    config.update(width=args.width, depth=args.depth)
    for key in ['n','n_layer','n_interface']:
        config[key] = int(config[key]*args.factor)
    if args.patch_expert:
        from Benchmarks.multiscale_patch import TracePatchPINN
        model = TracePatchPINN(domain, width=args.width, depth=args.depth)
    else:
        model = TracePINN(domain, width=args.width, depth=args.depth)
    ...
    if args.patch_expert:sources.append('multiscale_patch.py')
    ...
    config.update(variant='trace_fixed_features', solver='column-scaled SVD', rcond=1e-12,
                  ...
                  patch_expert=args.patch_expert,
