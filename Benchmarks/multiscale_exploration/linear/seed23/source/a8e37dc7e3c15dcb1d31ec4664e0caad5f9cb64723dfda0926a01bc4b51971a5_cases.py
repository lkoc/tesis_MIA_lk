...
El catálogo proporciona geometría y propiedades a full_domain.py, que resuelve
el conductor, las capas y el suelo explícitamente. Los ayudantes de resistencia
radial solo se conservan para reproducir la formulación reducida histórica.
Los casos no son reproducciones exactas de las pérdidas IEC de los artículos.
"""
...
def validate_case(c):
    ...
    if not finite(c):raise ValueError('Physical data must contain finite numbers')
    ...
    if c['kind']=='cable':
        if 'source_expression' in c or 'exact_expression' in c:raise ValueError('Cable soil model only supports Q=0 and boundary Joule heating; use MMS for prescribed volume sources')
        if not c['cables'] or c['current']<=0 or c['R20']<=0 or c['alpha']<0:raise ValueError('Invalid cable current, resistance or temperature coefficient')
