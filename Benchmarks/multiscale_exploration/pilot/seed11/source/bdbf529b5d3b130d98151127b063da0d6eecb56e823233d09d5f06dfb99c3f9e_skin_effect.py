...
def loss_specification(case):
    ...
    for field in ('resistance_ohm_m','frequency_Hz','mu_r','reference_temperature_C','source_temperature_C'):
        if not np.isfinite(spec[field]):raise ValueError(f'Electrical field {field} must be finite')
    ...
    elif spec.get('includes_proximity',False):
        raise ValueError('DC resistance and a skin-only correction do not include proximity losses')
