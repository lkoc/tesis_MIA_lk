...
def train(c,seed,args,out):
    ...
    def loss():
        ...
        return wp*lp+args.bc_weight*lb+args.flux_weight*lf+10*li+args.energy_weight*le+100*electrical+100*limit_loss,dict(pde=lp,bc=lb,flux=lf,interface=li,energy=le,electrical=electrical,temperature_limit=limit_loss)
...
def evaluate_model(model,meta,out):
    ...
    if f.exists():
        ...
        if model.ampacity:
            report.update(current_fem_A=fm['current_A'],error_current_pct=100*abs(report['current_A']/fm['current_A']-1),temperature_limit_error_K=abs(tm-meta.get('temperature_limit',90.)))
