...
SOURCE_SNAPSHOT={str(p.relative_to(SOURCE_ROOT)):p.read_bytes() for p in [Path(__file__),Path(__file__).with_name('cases.py'),Path(__file__).with_name('expressions.py')]}
...
def archive_previous(out,stem):
    """Preserve the exact prior JSON/field pair before a recalculation."""
    import shutil
    old=out/(stem+'.json')
    if old.exists():
        digest=hashlib.sha256(old.read_bytes()).hexdigest();folder=out/'evaluation_history';folder.mkdir(exist_ok=True)
        for suffix in ['.json','.npz']:
            src=out/(stem+suffix);dst=folder/(digest+suffix)
...
def solve(c,level,out):
    ...
    if c['kind'] in ('cable','annulus'):
        meta['boundary_diagnostics']=[]
        for j in range(len(c['cables']) if c['kind']=='cable' else 1):
            measured=-fem.assemble_scalar(fem.form(-k*ufl.dot(ufl.grad(uh),n)*ds(10+j)))
            meta['boundary_diagnostics'].append(dict(boundary=f'cable{j}' if c['kind']=='cable' else 'inner',prescribed_heat_into_soil_W_m=c['power'],computed_heat_into_soil_W_m=measured,relative_power_error_pct=100*abs(measured/c['power']-1)))
    if c['kind']!='cable':
    ...
    meta['source_sha256']={name:hashlib.sha256(data).hexdigest() for name,data in SOURCE_SNAPSHOT.items()}
    meta.update(completed_utc=datetime.now(timezone.utc).isoformat(),mpi_processes=MPI.COMM_WORLD.size,numpy=np.__version__,ufl=ufl.__version__)
    ...
    archive_previous(out,f'fem_l{level}')
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--legacy-reduced',action='store_true');ap.add_argument('--cases',nargs='+',default=['all']);ap.add_argument('--levels',nargs='+',type=int,default=[0,1,2]);ap.add_argument('--output',default='Benchmarks/results');args=ap.parse_args()
    ...
    if not args.legacy_reduced and any(catalog[name]['kind']=='cable' for name in names):
        ap.error('Use full_fem.py for explicit cable physics; --legacy-reduced is historical reproduction only')
    for name in names:
