...
def fem_root(case,level,output,limit=90.):
    ...
    record.update(physics_sha256=problem.fingerprint,specification=problem.specification(),solution_current_A=current,
        ...
        method='explicit_multimaterial_FEM_with_DC_current_root')
...
def train_ampacity(case,config,reference,output,limit=90.):
    ...
    model=Model();objective=Loss(model,config['n'],config['n_layer'],config['n_interface'],config['sampling_seed'],config['sampling'])
    ...
    source={};archive=output/'source';archive.mkdir(exist_ok=True)
    source_paths=[Path(__file__).with_name(name) for name in ['full_ampacity.py','full_pinn.py','full_domain.py','full_physics.py','full_sampling.py','skin_effect.py','cases.py','expressions.py']]+[ROOT/'pinn_cables/pinn/pde.py']
    ...
    (output/f'training_seed{seed}.json').write_text(json.dumps(dict(configuration=config,source_sha256=source,history=history,training_seconds=time.perf_counter()-start,parameters=sum(p.numel() for p in model.parameters() if p.requires_grad),fem_labels_used=0,python=sys.version,torch=torch.__version__,platform=platform.platform(),device='cpu',dtype='float64',threads=1),indent=2),encoding='utf-8')
    print(json.dumps(dict(current_A=current,accepted=report['accepted'])),flush=True);return report

if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--fem',action='store_true');parser.add_argument('--case',required=True);parser.add_argument('--output',type=Path,required=True);parser.add_argument('--config',type=Path);parser.add_argument('--reference');parser.add_argument('--seed',type=int,default=71)
    args=parser.parse_args();case=ampacity_cases()[args.case]
    if args.fem:
        records=[fem_root(case,level,args.output) for level in [0,1,2]]
        ...
        selected=json.loads(args.config.read_text());config=dict(selected.get('configuration',selected),seed=args.seed,sampling_seed={71:1001,83:1003,97:1007}[args.seed])
        train_ampacity(case,config,args.reference,args.output)
