...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--legacy-reduced',action='store_true');ap.add_argument('--cases',nargs='+',default=['all']);ap.add_argument('--levels',nargs='+',type=int,default=[0,1,2]);ap.add_argument('--output',type=Path,default=ROOT/'Benchmarks/coupled_results');args=ap.parse_args()
    if not args.legacy_reduced:ap.error('Use full_fem.py --mode dc_temperature; this solver is historical and requires --legacy-reduced')
