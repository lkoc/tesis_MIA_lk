...
class Network(nn.Module):
    ...
    def plain(self,xy):
        ...
        if len(self.nets)==2:
            axis=0 if c.get('interface',{}).get('axis','x')=='x' else 1
            return torch.where(xy[:,axis:axis+1]<=c.get('interface',{}).get('position',.5),self.nets[0](z),self.nets[1](z))
    ...
    def forward(self,xy):
        ...
        if hasattr(self,'multipoles'): result=result+self.basis(xy)@self.multipoles.reshape(-1,1)
        return result
...
def train(c,seed,args,out):
    ...
    if c['kind']=='mms_interface':
        ...
        iaxis=0 if c.get('interface',{}).get('axis','x')=='x' else 1
        pos=c.get('interface',{}).get('position',.5)
        ip=np.c_[np.full(128,pos),yi] if iaxis==0 else np.c_[yi,np.full(128,pos)]
        inter=tensor(ip,True)
    ...
    def loss():
        ...
        if c['kind']=='annulus':
            ...
            if interior_basis is not None: residual=residual+scale*(interior_basis[2]@model.multipoles.reshape(-1,1))
        ...
        for z,normal,edge,bz,gz,kz,target,zbasis in cached:
            ...
            if zbasis is not None:
                coef=model.multipoles.reshape(-1,1)
        ...
        if c['kind']=='mms_interface':
            ...
            li=torch.mean((ul-ur)**2)+torch.mean((kminus*gl-kplus*gr)**2)
...
def evaluate_model(model,meta,out):
    ...
    if c['kind']=='mms_interface':
        yy=np.linspace(.0001,.9999,512);iaxis=0 if c.get('interface',{}).get('axis','x')=='x' else 1;pos=c.get('interface',{}).get('position',.5)
        ip=np.c_[np.full(512,pos),yy] if iaxis==0 else np.c_[yy,np.full(512,pos)]
        z=tensor(ip,True)
