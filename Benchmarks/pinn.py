...
SOURCE_SNAPSHOT={str(p.relative_to(SOURCE_ROOT)):p.read_bytes() for p in [Path(__file__),Path(__file__).with_name('cases.py'),Path(__file__).with_name('expressions.py'),Path(__file__).with_name('electrothermal.py'),Path(__file__).with_name('electrical_model.json'),Path(__file__).with_name('adaptive.py'),SOURCE_ROOT/'pinn_cables/pinn/pde.py']}
...
def train(c,seed,args,out):
    ...
    history=[];adaptive_history=[];initial_xy=xy.detach().numpy().copy();clouds={'initial_xy':initial_xy}
    opt=torch.optim.Adam(model.parameters(),lr=args.lr)
    for step in range(args.adam):
        if args.sampling!='fixed' and step in args.adapt_steps:
            sampling_seed=seed+100000+step
            index=len(adaptive_history)
            clouds.update({f'round{index}_state_{key}':value.detach().numpy().copy() for key,value in model.state_dict().items()})
            candidates=training_points(c,sampling_seed,args.n*args.candidate_multiplier);scores=[]
            for start_score in range(0,len(candidates),512):
                probe=tensor(candidates[start_score:start_score+512],True);temperature=predict(model,probe)
                if args.sampling=='gradient':score=torch.linalg.vector_norm(gradients(temperature,probe),dim=1)/c['scale']
                elif args.sampling=='residual':
                    conductivity=field_k(c,probe[:,:1],probe[:,1:2],torch)
                    score=torch.abs((laplace_variable_k(temperature,probe,conductivity)+source(c,probe[:,:1],probe[:,1:2],torch))/(conductivity*c['scale'])).ravel()
                else:score=probe[:,0]*0+1.
                scores.append(score.detach().numpy())
            scores=np.concatenate(scores)
            points,selection=resample(initial_xy,candidates,scores,sampling_seed,args.sampling,args.adaptive_fraction)
            index=len(adaptive_history);clouds.update({f'round{index}_xy':points,f'round{index}_candidates':candidates,f'round{index}_scores':scores,**{f'round{index}_{key}':value for key,value in selection.items()}})
            adaptive_history.append(dict(step=step,mode=args.sampling,seed=sampling_seed,candidate_count=len(candidates),interior_count=len(points),mean_score=float(scores.mean()),selected_mean_score=float(scores[selection['selected_indices']].mean())))
            xy=tensor(points,True);k=field_k(c,xy[:,:1],xy[:,1:2],torch)
            gk=gradients(k,xy).detach() if k.requires_grad else torch.zeros_like(xy)
            bg=background(c,xy,args.variant)
            rbg=laplace_variable_k(bg,xy,k).detach() if c['kind']=='cable' and args.variant!='direct' else torch.zeros_like(k)
            q=source(c,xy[:,:1],xy[:,1:2],torch).detach();kd=k.detach();interior_basis=basis_cache(xy,k)
    ...
    meta['sampling']=args.sampling;meta['adaptive_history']=adaptive_history
    ...
    previous=out/f'pinn_seed{seed}.pt'
    if previous.exists():
        import shutil
        saved=out/'run_history'/(str(seed)+'_'+hashlib.sha256(previous.read_bytes()).hexdigest());saved.mkdir(parents=True,exist_ok=True)
        for old,new in [(f'pinn_seed{seed}.pt','weights.pt'),(f'pinn_seed{seed}.json','metrics.json'),(f'pinn_seed{seed}.npz','fields.npz'),(f'training_seed{seed}.json','training.json'),(f'collocation_seed{seed}.npz','collocation.npz')]:
            if (out/old).exists() and not (saved/new).exists():shutil.copy2(out/old,saved/new)
    ...
    clouds['final_xy']=xy.detach().numpy();np.savez_compressed(out/f'collocation_seed{seed}.npz',**clouds)
...
def evaluate_model(model,meta,out):
    ...
    flux=0.;throughput=0.;bc_error=[];boundary_diagnostics=[]
    for points,normals,edge in boundary_points(c,512):
        ...
        diagnostic=dict(boundary=edge,net_outward_heat_W_m=float(qn.mean()*length))
        if edge.startswith('cable') or (c['kind']=='annulus' and edge=='inner'):
            prescribed=float(powers[int(edge[5:])]) if edge.startswith('cable') else c['power']
            target=-prescribed/length;error=qn-target
            diagnostic.update(prescribed_heat_into_soil_W_m=prescribed,computed_heat_into_soil_W_m=float(-qn.mean()*length),relative_power_error_pct=100*abs(float(-qn.mean()*length)/prescribed-1),flux_rmse_W_m2=float(np.sqrt(np.mean(error**2))),flux_max_error_W_m2=float(max(abs(error))))
        else:
            temperature_error=T.detach().numpy().ravel()-(exact(c,points[:,0],points[:,1]) if c['kind'].startswith('mms') else c['T0'])
            diagnostic.update(temperature_rmse_K=float(np.sqrt(np.mean(temperature_error**2))),temperature_max_error_K=float(max(abs(temperature_error))))
            if c['kind']=='mms_robin' and edge=='top':
                tinf=exact(c,points[:,0],points[:,1])+3*points[:,0]*(1-points[:,0]);error=qn-10*(T.detach().numpy().ravel()-tinf)
                diagnostic.update(robin_flux_rmse_W_m2=float(np.sqrt(np.mean(error**2))),robin_flux_max_error_W_m2=float(max(abs(error))))
    ...
    report['boundary_diagnostics']=boundary_diagnostics
    if model.coupled:
    ...
    f=out/fname
    if f.exists():
        local_meta=json.loads(f.with_suffix('.json').read_text())
        if bool(local_meta.get('coupled',False))!=model.coupled or bool(local_meta.get('ampacity',False))!=model.ampacity:
            f=Path(__file__).resolve().parent/('coupled_results' if model.coupled else 'results')/c['id']/fname
    else:f=Path(__file__).resolve().parent/('coupled_results' if model.coupled else 'results')/c['id']/fname
    if f.exists():
        femdata=np.load(f)
        ...
        if bool(fm.get('coupled',False))!=model.coupled or bool(fm.get('ampacity',False))!=model.ampacity:raise ValueError('FEM/PINN electrical modes differ')
        if model.coupled and any(fm['loss_model'][key]!=meta['loss_model'][key] for key in ['type','reference_temperature_C']):raise ValueError('FEM/PINN loss laws differ')
    ...
    report['evaluation']={'completed_utc':datetime.now(timezone.utc).isoformat(),'source_sha256':{p:hashlib.sha256(data).hexdigest() for p,data in SOURCE_SNAPSHOT.items()},'fem_reference':str(f.resolve()),'fem_json_sha256':hashlib.sha256(f.with_suffix('.json').read_bytes()).hexdigest() if f.exists() else None}
    for name,data in SOURCE_SNAPSHOT.items():
        archive=out/'source'/(hashlib.sha256(data).hexdigest()+'_'+Path(name).name)
...
def main():
    ...
    ap.add_argument('--legacy-reduced',action='store_true',help='Historical reduced cable reproduction only')
    ...
    ap.add_argument('--sampling',choices=['fixed','uniform','residual','gradient'],default='fixed')
    ap.add_argument('--adapt-steps',nargs='+',type=int,default=[400,800])
    ap.add_argument('--candidate-multiplier',type=int,default=4)
    ap.add_argument('--adaptive-fraction',type=float,default=.5)
    args=ap.parse_args()
    if args.config:
    ...
    if args.sampling not in ('fixed','uniform','residual','gradient'):raise ValueError('Unknown sampling mode')
    if args.sampling!='fixed':
        if not 0<args.adaptive_fraction<=1:raise ValueError('Adaptive fraction must be in (0,1]')
        if not isinstance(args.candidate_multiplier,int) or args.candidate_multiplier<1:raise ValueError('Candidate multiplier must be a positive integer')
        if not args.adapt_steps or any(not isinstance(s,int) or s<=0 or s>=args.adam for s in args.adapt_steps) or args.adapt_steps!=sorted(set(args.adapt_steps)):raise ValueError('Distinct increasing adaptive steps must lie inside the Adam phase')
    torch.set_default_dtype(torch.float64)
    torch.set_num_threads(args.threads);catalog=cases();names=list(catalog) if args.cases==['all'] else args.cases
    if not args.legacy_reduced and any(catalog[name]['kind']=='cable' for name in names):
        ap.error('Cable physics moved to full_pinn.py (all variants) and run_case.py (FEM + PINN). Use --legacy-reduced only to reproduce historical evidence.')
    for name in names:
        if args.sampling!='fixed' and catalog[name]['kind']=='annulus':raise ValueError('Radial annulus uses fixed one-dimensional residual points; adaptive sampling is not implemented for this reduction')
