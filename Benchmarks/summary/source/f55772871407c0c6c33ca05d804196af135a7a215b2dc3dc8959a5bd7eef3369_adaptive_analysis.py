...
def narrative(result):
    ...
    for name in STUDIES:
        ...
        descriptions={'uniform':'la renovación aleatoria','residual':'la selección por residuo','gradient':'la selección por gradiente'}
        alternatives='; '.join(f"{descriptions[mode]} obtiene {rr[mode]['median_rmse_K']:.4f} K y {rr[mode]['accepted']}/3 aceptadas" for mode in MODES[1:])+'.'
        findings+=alternatives[0].upper()+alternatives[1:]
        ...
        if selected_mode:
            ...
            lines+=['',decision];text+='\n'+decision+'\n'
            selected=rr[selected_mode];reduction=100*(1-selected['median_rmse_K']/base['median_rmse_K'])
            if STUDIES[name]['family']=='nominal':
                conclusions.append(f"En el arreglo estratificado de seis cables, ninguna redistribución supera una de tres semillas aceptadas. La renovación aleatoria seleccionada mantiene prácticamente el RMSE mediano de la nube fija y reduce su dispersión; priorizar el residuo o el gradiente no mejora el cumplimiento conjunto en este caso.")
            else:
                conclusions.append(f"En la corriente límite del XLPE con relleno mejorado, la selección por residuo reduce el RMSE mediano en {reduction:.2f}\\,\\pct{{}} y eleva la aceptación de {base['accepted']} a {selected['accepted']} de tres semillas. El peor error de corriente permanece en {selected['max_current_error_pct']:.2f}\\,\\pct{{}}, por lo que la mejora es parcial y debe conservarse la comprobación individual frente a FEM.")
