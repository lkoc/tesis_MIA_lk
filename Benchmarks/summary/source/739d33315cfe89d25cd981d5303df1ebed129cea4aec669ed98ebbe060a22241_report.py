...
def records():
    ...
    for p in sorted(BASE.rglob('pinn_seed*.json')):
        ...
        row.update(sampling=m.get('sampling','fixed'),adam=m['adam'],lbfgs=m['lbfgs_max_iter'],n_interior=m['n_interior'],electrical_weight=m.get('configuration',{}).get('electrical_weight',100.) if m.get('coupled') else None,limit_weight=m.get('configuration',{}).get('limit_weight',100.) if m.get('ampacity') else None)
...
def selection_tables(rows,tables,figures):
    ...
    labels.update({f'adaptive_{family}_{mode}':label for family in ['nominal','ampacity'] for mode,label in [('uniform','Renovación aleatoria'),('residual','Residuo adaptativo'),('gradient','Gradiente adaptativo')]})
    ...
    latex_table(tables/'benchmark_ampacity_selection.tex',['Caso / alternativa','RMSE (K)',r'$E_{I,\max}$ (\%)',r'$|T_{\max}-90|$ (K)','Aceptadas'],amp,'Comparación de configuraciones para corriente límite con igual presupuesto; errores máximos entre semillas','tab:seleccion-corriente')
    import matplotlib.pyplot as plt
