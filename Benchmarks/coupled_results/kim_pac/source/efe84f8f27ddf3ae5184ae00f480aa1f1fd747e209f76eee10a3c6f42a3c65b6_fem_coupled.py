...
from Benchmarks.fem import create_mesh,evaluate,UFLBackend
...
SNAPSHOT={str(p.relative_to(ROOT)):p.read_bytes() for p in [Path(__file__),Path(__file__).with_name('fem.py'),Path(__file__).with_name('cases.py'),Path(__file__).with_name('expressions.py'),Path(__file__).with_name('electrothermal.py')]}
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--cases',nargs='+',default=['all']);ap.add_argument('--levels',nargs='+',type=int,default=[0,1,2]);args=ap.parse_args()
    for name,c in cases().items():
        if c['kind']=='cable' and (args.cases==['all'] or name in args.cases):
            for level in args.levels:solve(c,level,ROOT/'Benchmarks/coupled_results'/name)
